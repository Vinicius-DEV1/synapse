...
export default function DictionaryModal({
  ...
  const [mode, setMode] = useState<'offline' | 'online'>(
    settings.dictionaryMode || (settings.hasOfflineDictionary ? 'offline' : 'online')
  );
  ...
  return (
    <Modal
      ...
      containerClassName="dictionary-modal-container select-none w-[600px] max-w-[95vw] max-h-[85dvh] flex flex-col"
    >
      <div className="bg-dark-card border border-white/10 rounded-2xl w-full max-h-[85dvh] flex flex-col shadow-2xl overflow-hidden animate-scale-in">
                ...
                <button
                  ...
                  className={`text-xs font-semibold pb-1 border-b-2 transition-colors ${
                    languageTab === 'en'
                      ...
                      : 'border-transparent text-dark-subtext hover:text-white'
                  }`}
                >
                  🇺🇸 English
                </button>
        ...
        <div className="px-4 py-2 bg-dark-bg/80 flex items-center justify-between border-t border-white/5">
          ...
          <div className="flex items-center gap-2">
            ...
            {dictionaryData &&
              (dictionaryData.english?.anki_card || dictionaryData.portuguese?.anki_card) && (
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        (() => {
          const cardDraft: CardDraft & {
            ...
            video_clip:
              sourceType === 'video' &&
              dictionaryData.detected_language === 'en' &&
              dictionaryData.english?.anki_card?.video_clip
                ? {
                    path: videoClip!.path,
                    startMs: dictionaryData.english.anki_card.video_clip.startMs,
                    endMs: dictionaryData.english.anki_card.video_clip.endMs,
                  }
                : sourceType === 'video' &&
                    dictionaryData.detected_language === 'pt' &&
                    dictionaryData.portuguese?.anki_card?.video_clip
                  ? {
                      path: videoClip!.path,
                      startMs: dictionaryData.portuguese.anki_card.video_clip.startMs,
                      endMs: dictionaryData.portuguese.anki_card.video_clip.endMs,
                    }
                  : videoClip,
          };
          return (
            <CardEditor draft={cardDraft} onClose={() => setShowAnkiEditor(false)} />
          );
        })()}
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <ul className="list-disc pl-4 space-y-2 marker:text-brand-500/50">
                {selectedColloc.examples.map((ex, i) => (
                  <li key={i} className="text-sm italic text-dark-text/90 leading-relaxed">
                    "{ex}"
                  </li>
                ))}
              </ul>
