...
import { Plus, MessageCircle, Clock, Trash2, Mic, Menu, X, Briefcase } from 'lucide-react';
import type { TutorSession, InterviewConfig } from '../../types';
import { isInterviewSession } from '../../types';
import PracticeChat from './PracticeChat';
import { NewPracticeSessionModal } from './modals/NewPracticeSessionModal';
import { InterviewSetupModal } from './modals/InterviewSetupModal';
...
export default function PracticeView() {
  ...
  const handleCreateInterviewSession = async (config: InterviewConfig) => {
    setIsInterviewSetupOpen(false);
    if (!window.api?.practice) return;
    try {
      const newSession = await window.api.practice.createSession({
        title: `Entrevista: ${config.jobTitle}`,
        started_at: new Date().toISOString(),
        custom_prompt: JSON.stringify(config),
      });
      setSessions((prev) => [newSession, ...prev]);
      setActiveSession(newSession);
    } catch (err) {
  ...
  return (
    <div className="flex h-full bg-dark-bg text-dark-text relative">
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    {isInterview ? (
                      <Briefcase size={14} className={activeSession?.id === session.id ? 'text-brand-400 shrink-0' : 'text-sky-400 shrink-0'} />
                    ) : (
                      <MessageCircle size={14} className={activeSession?.id === session.id ? 'text-brand-400 shrink-0' : 'text-dark-subtext shrink-0'} />
                    )}
                    <span className="text-sm font-medium truncate pr-2">{session.title}</span>
                  </div>
                  <button
                    onClick={(e) => handleDeleteSession(session.id, e)}
                    className="opacity-0 group-hover:opacity-100 p-1 text-dark-subtext hover:text-red-400 transition-opacity shrink-0"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
                <div className="flex items-center gap-1 mt-1 text-[10px] text-dark-subtext ml-5">
                  ...
                  {dateStr} às {timeStr}
                  {isInterview && (
                    <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded bg-brand-500/20 text-brand-300 font-semibold uppercase">
                      Entrevista
                    </span>
                  )}
                </div>
      ...
      <div className={`flex-1 flex flex-col relative overflow-hidden bg-dark-bg/50 ${isMobile ? '' : 'ml-0'}`}>
        {activeSession ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center opacity-40 select-none">
            ...
            <h1 className="text-2xl font-bold tracking-tight mb-2">Prática & Entrevistas com IA</h1>
            <p className="text-sm">Selecione uma sessão ou crie uma nova para conversar ou simular entrevistas.</p>
          </div>
        )}
      </div>

      {/* Modal de Escolha do Tipo de Sessão */}
      <NewPracticeSessionModal
        isOpen={isNewModalOpen}
        onClose={() => setIsNewModalOpen(false)}
        onSelectFreeMode={handleCreateFreeSession}
      ...
      {/* Modal de Configuração da Entrevista */}
      <InterviewSetupModal
        ...
        onSubmit={handleCreateInterviewSession}
      />
    </div>
  );
}
