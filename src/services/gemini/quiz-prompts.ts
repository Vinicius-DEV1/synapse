...
export function buildQuizValidationPrompt(
  ...
  let prompt = `You are a rigorous, highly skeptical Academic Fact-Checker, Senior Tech Reviewer, and Pedagogical Expert.
Your sole mission is to audit, verify, fact-check, and eliminate hallucinations from candidate study questions generated by an initial AI model before they are delivered to the student.

--- CANDIDATE QUESTIONS TO AUDIT & VERIFY ---
${JSON.stringify(candidateQuestions, null, 2)}
`;

  if (userMessageOrTopic) {
    prompt += `\nOriginal User Request / Topic: "${userMessageOrTopic}"\n`;
  }

  if (contextText && contextText.trim()) {
    prompt += `Reference Material Context: "${contextText.trim().slice(0, 1500)}"\n`;
  }

  prompt += `
MANDATORY VERIFICATION & FACT-CHECKING CRITERIA:
1. FACTUAL ACCURACY & ZERO HALLUCINATION (CRITICAL):
   - Scrutinize every technical assertion, term, framework API, class, method signature, historical date, formula, and conceptual rule.
   - If the first AI hallucinated a non-existent method, deprecated syntax, or inaccurate scientific/historical fact, you MUST correct it immediately.
   - Base all technical content strictly on official, modern, current standards.
2. ANSWER INTEGRITY & DISTRACTORS:
   - For Multiple Choice:
     - Verify that the designated correct option (correctIndex) is indisputably and completely accurate.
     - Verify that the 3 distractors are definitively incorrect, but plausible. Ensure there is NO ambiguity or accidental multiple correct answers.
     - If the correct index was wrong, fix "correctIndex" and/or fix the option texts.
   - For Open-Ended:
     - Verify that "expectedAnswer" is direct, technically sound, and contains all key concepts needed for evaluation.
3. EXPLANATION QUALITY:
   - Ensure "explanation" teaches the core concept with clarity and substantiates why the answer is true.
4. LANGUAGE CONSISTENCY:
   - Keep the exact same language as the candidate questions (Portuguese if in Portuguese, English if in English). Do not translate between languages unless asked.

OUTPUT FORMAT:
Respond STRICTLY in raw JSON matching this schema:
{
  "validatedQuestions": [
    {
      "type": "multiple_choice" | "open",
      "question": "Fact-checked, clear question text",
      "options": ["Option 1", "Option 2", "Option 3", "Option 4"],
      "correctIndex": 0,
      "expectedAnswer": "Model answer for open questions",
      "explanation": "Didactic, accurate explanation",
      "factCheckVerdict": "approved" | "corrected",
      "improvements": "Brief note explaining what was corrected, or 'Fatos verificados sem inconsistências' if perfect."
    }
  ],
  "validationSummary": "A concise pedagogical summary of the fact-checking pass."
}

Do NOT wrap in markdown code blocks (\`\`\`json). Return raw JSON only.`;
