...
pub fn diagrams_get_all(db_state: State<'_, DbState>) -> Result<Vec<DiagramMeta>, String> {
    ...
    let diagram_iter = stmt.query_map([], |row| {
        Ok(DiagramMeta {
            id: row.get(0)?,
            title: row.get(1)?,
            icon: row.get(2)?,
...
#[tauri::command]
pub fn diagrams_get_content(id: String, db_state: State<'_, DbState>) -> Result<DiagramContent, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
    
    let mut stmt = conn.prepare("SELECT content, encrypted_content FROM diagrams WHERE id = ?")
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn diagrams_create(payload: CreateDiagramPayload, db_state: State<'_, DbState>) -> Result<DiagramMeta, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
...
#[tauri::command]
pub fn diagrams_update(payload: UpdateDiagramPayload, db_state: State<'_, DbState>) -> Result<i32, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
