...
#[tauri::command]
pub fn practice_create_session(session: TutorSession, db_state: State<'_, DbState>) -> Result<TutorSession, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
...
#[tauri::command]
pub fn practice_update_session(session: TutorSession, db_state: State<'_, DbState>) -> Result<i32, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
...
#[tauri::command]
pub fn practice_get_messages(session_id: String, db_state: State<'_, DbState>) -> Result<Vec<TutorMessage>, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
...
#[tauri::command]
pub fn practice_create_message(message: TutorMessage, db_state: State<'_, DbState>) -> Result<TutorMessage, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
    
    let id = if message.id.is_empty() { uuid::Uuid::new_v4().to_string() } else { message.id.clone() };
...
#[tauri::command]
pub fn practice_create_memory(memory: TutorMemory, db_state: State<'_, DbState>) -> Result<TutorMemory, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
    
    let id = if memory.id.is_empty() { uuid::Uuid::new_v4().to_string() } else { memory.id.clone() };
