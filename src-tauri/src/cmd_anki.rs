...
#[tauri::command]
pub fn anki_get_deck_settings(deck_id: String, db_state: State<'_, DbState>) -> Result<AnkiDeckSettings, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
    ...
    let row = stmt.query_row([&deck_id], |row| {
        Ok(AnkiDeckSettings {
            new_limit: row.get(0)?,
...
pub fn anki_get_all_cards(deck_id: Option<String>, db_state: State<'_, DbState>) -> Result<Vec<AnkiCard>, String> {
    ...
    let iter = stmt.query_map(params_iter, |row| {
        Ok(AnkiCard {
            id: row.get(0)?, deck_id: row.get(1)?, front: row.get(2)?, back: row.get(3)?, extra_note: row.get(4)?,
...
#[tauri::command]
pub fn anki_review_card_fsrs(card_id: String, rating: i32, state: FsrsState, db_state: State<'_, DbState>) -> Result<bool, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
...
#[tauri::command]
pub fn anki_update_deck(deck_id: String, name: String, description: Option<String>, db_state: State<'_, DbState>) -> Result<bool, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
    conn.execute(
...
#[tauri::command]
pub fn anki_update_deck_settings(deck_id: String, settings: AnkiDeckSettings, db_state: State<'_, DbState>) -> Result<bool, String> {
    let guard = db_state.conn.lock().unwrap();
    let conn = guard.as_ref().ok_or("Banco não inicializado")?;
