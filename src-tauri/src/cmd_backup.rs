...
fn decrypt_database_copy(
    ...
    let keys = keys
        .as_ref()
        .ok_or("Chaves não disponíveis — faça login primeiro")?;

    let conn =
        rusqlite::Connection::open(db_path).map_err(|e| format!("Falha ao abrir cópia: {}", e))?;
    ...
    if let Some(ref notes_key) = keys.notes {
        let mut stmt = conn
            .prepare(
                "SELECT id, encrypted_content FROM diagrams WHERE encrypted_content IS NOT NULL",
            )
            .map_err(|e| e.to_string())?;
...
fn download_drive_file(
    ...
    fs::write(&dest_file, &bytes).map_err(|e| format!("Falha ao salvar {}: {}", safe_name, e))?;
